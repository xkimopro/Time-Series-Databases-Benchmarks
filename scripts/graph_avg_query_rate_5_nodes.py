...
for ds_size in dataset_sizes[:1]:
    for q in queries:
        ...
        with open(f"../cluster_performance/query_5_nodes/ten_queries/{db_name[0]}/{ds_size}/{db_name[0]}-queries-{q}-{ds_size}-10-queries.json") as json_file:
            data = json.load(json_file)
            time_avg_q_rate[dataset_sizes.index(
                ds_size)] += data['Totals']['overallQueryRates']['all_queries']

        with open(f"../cluster_performance/query_5_nodes/ten_queries/{db_name[1]}/{ds_size}/{db_name[1]}-queries-{q}-{ds_size}-10-queries.json") as json_file:
            data = json.load(json_file)
...
for ds_size in dataset_sizes:
    for q in queries:
        ...
        with open(f"../cluster_performance/query_5_nodes/single_query/{db_name[0]}/{ds_size}/{db_name[0]}-queries-{q}-{ds_size}-1-queries.json") as json_file:
            data = json.load(json_file)
            time_avg_q_rate[dataset_sizes.index(
                ds_size)] += data['Totals']['overallQueryRates']['all_queries']

        with open(f"../cluster_performance/query_5_nodes/single_query/{db_name[1]}/{ds_size}/{db_name[1]}-queries-{q}-{ds_size}-1-queries.json") as json_file:
            data = json.load(json_file)
            inf_avg_q_rate[dataset_sizes.index(
                ds_size)] += data['Totals']['overallQueryRates']['all_queries']
